...
Two properties matter more than throughput:
  ...
  * Generating is proposing, never publishing. No file says whether its
    content is live: each instance serves only what an administrator has
    approved on its own review page, so a generated item reaches no pupil
    until someone has met it there.
...
def to_items(goal_code: str, drafted: list[dict[str, Any]]) -> tuple[list[QuizItem], list[str]]:
    ...
    repair is a question nobody has read properly, and it would arrive in the
    review queue looking exactly like a sound one.
    """
    items: list[QuizItem] = []
    rejected: list[str] = []

    for index, draft in enumerate(drafted, start=1):
        payload = {**draft, "id": f"{goal_code}-{index:02d}", "goal": goal_code}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if destination.exists():
        # Never clobber authored work. Re-running writes alongside so a human
    ...
    header = (
        ...
        f"# Nothing here is served until an administrator approves it on an\n"
        f"# instance's review page. Questions here are ours, not Udir's.\n"
    )
    destination.write_text(
        header + yaml.safe_dump(payload, allow_unicode=True, sort_keys=False), encoding="utf-8"
    )

    print(f"\nWrote {len(all_items)} item(s) to {destination.relative_to(REPO_ROOT)}")
    print("Read them, then run: uv run python -m pensum.items.validate")
    return 0
